...
internal sealed class PaymentLedgerStorage
{
    ...
    private void UpsertPayment(WorkspaceEvent workspaceEvent, RuntimeDbSession db, string status)
    {
        ...
        command.Parameters.AddWithValue("payer", Value(workspaceEvent, "付款人", "张三"));
        command.Parameters.AddWithValue("amount", NpgsqlDbType.Numeric, DecimalValue(workspaceEvent, "收款金额", DecimalValue(workspaceEvent, "确认金额", 3000m)));
        command.Parameters.AddWithValue("currency", Value(workspaceEvent, "币种", "KGS"));
        command.Parameters.AddWithValue("method", Value(workspaceEvent, "支付方式", "现金"));
        command.Parameters.AddWithValue("purpose", Value(workspaceEvent, "收款用途", "房租"));
        command.Parameters.AddWithValue("receiptNo", Value(workspaceEvent, "收款凭证", workspaceEvent.EventId));
    ...
    private void UpsertFinanceReconciliation(WorkspaceEvent workspaceEvent, RuntimeDbSession db, string status)
    {
        ...
        command.Parameters.AddWithValue("workspaceId", workspaceEvent.WorkspaceId);
        command.Parameters.AddWithValue("paymentId", Value(workspaceEvent, "收款记录", StableId("payment", workspaceEvent)));
        command.Parameters.AddWithValue("channel", Value(workspaceEvent, "银行/钱包渠道", "现金"));
        command.Parameters.AddWithValue("confirmedAmount", NpgsqlDbType.Numeric, confirmedAmount);
        command.Parameters.AddWithValue("currency", Value(workspaceEvent, "币种", "KGS"));
        command.Parameters.AddWithValue("matchResult", Value(workspaceEvent, "确认结果", "确认"));
        command.Parameters.AddWithValue("varianceAmount", NpgsqlDbType.Numeric, DecimalValue(workspaceEvent, "差异金额", 0m));
        command.Parameters.AddWithValue("status", status);
    ...
    private void AppendAllocation(WorkspaceEvent workspaceEvent, RuntimeDbSession db, string status)
    {
        ...
        command.Parameters.AddWithValue("paymentId", Value(workspaceEvent, "收款记录", StableId("payment", workspaceEvent)));
        command.Parameters.AddWithValue("workspaceId", workspaceEvent.WorkspaceId);
        command.Parameters.AddWithValue("allocationMode", Value(workspaceEvent, "分配方式", "自动抵最早欠款"));
        command.Parameters.AddWithValue("allocatedAmount", NpgsqlDbType.Numeric, DecimalValue(workspaceEvent, "分配金额", 0m));
        command.Parameters.AddWithValue("status", status);
        command.Parameters.AddWithValue("createdEventId", workspaceEvent.EventId);
