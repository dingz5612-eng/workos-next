...
internal static class OptionSetRegistry
{
    ...
    private static IReadOnlyList<(string Value, string ZhLabel)> StableOptions(string optionSet) => optionSet switch
    {
        ...
        "approvedApplicationCandidates" => new[] { ("runtime_approved_application", "运行时审批申请"), ("runtime_pending_application", "待审批申请"), ("new", "新审批申请") },
        "leadCandidates" => new[] { ("runtime_lead", "运行时线索"), ("runtime_follow_up_lead", "待跟进线索"), ("new", "新线索") },
        "reservationCandidates" => new[] { ("runtime_reservation", "运行时预订单"), ("runtime_pending_reservation", "待转换预订"), ("pending", "待转换预订") },
        "stayCandidates" => new[] { ("runtime_stay", "运行时入住单"), ("runtime_active_stay", "当前在住"), ("current", "当前在住") },
        "depositCandidates" => new[] { ("runtime_deposit", "运行时押金单"), ("runtime_pending_deposit", "待处理押金"), ("pending", "待处理押金") },
        "depositReceiptCandidates" => new[] { ("runtime_deposit_receipt", "运行时押金收款"), ("runtime_pending_deposit_receipt", "待确认押金收款"), ("pending", "待确认押金收款") },
        "chargeCandidates" => new[] { ("runtime_charge", "运行时应收项"), ("runtime_pending_charge", "待分配应收"), ("oldest_debt", "最早欠款") },
        "serviceTaskCandidates" => new[] { ("runtime_service_task", "运行时服务任务"), ("runtime_pending_service_task", "待完成任务"), ("pending", "待完成任务") },
        "expenseCandidates" => new[] { ("runtime_expense", "运行时支出记录"), ("runtime_pending_expense", "待审批支出"), ("pending", "待审批支出") },
        "periodCandidates" => new[] { ("runtime_period", "运行时经营周期"), ("runtime_current_period", "当前周期"), ("current", "当前周期") },
        "roomCandidates" => new[] { ("runtime_room", "运行时房间"), ("runtime_available_room", "可用房间"), ("new", "新房间") },
        "bedCandidates" => new[] { ("runtime_bed", "运行时床位"), ("runtime_available_bed", "可用床位"), ("new", "新床位") },
        "roomBedCandidates" => new[] { ("runtime_room_bed", "运行时房间床位"), ("runtime_available_room_bed", "可用房间床位"), ("new", "新房间床位") },
        "residentCandidates" => new[] { ("runtime_resident", "运行时入住人"), ("runtime_active_resident", "当前入住人"), ("new", "新入住人") },
        "paymentCandidates" => new[] { ("runtime_payment", "运行时收款记录"), ("runtime_pending_payment", "待匹配支付"), ("pending", "待匹配支付") },
        "customerCandidates" => new[] { ("runtime_customer", "运行时客户"), ("runtime_fleet_partner", "车队客户"), ("new", "新客户") },
