...
public sealed class AdmissionKernelService
{
        ...
        bool productionRequested)
    {
        ...
        if (productionRequested)
        {
            return AdmissionKernelDecision.Blocked(
                ...
                new[] { "docs/oma/current-admission-state.json", "docs/business/business-line-registry.json", "docs/contracts/definition/workitem-definition-registry.json" },
                requiredCapabilities,
                requiredDeviceTrust,
                businessLine.Level,
                businessLine.SurfaceMode,
            ...
            !requiredOwnerRoles.Contains(actor.Role, StringComparer.OrdinalIgnoreCase))
        {
            return AdmissionKernelDecision.Blocked(
                ...
                new[] { "docs/oma/current-architecture.md", "docs/contracts/admission/admission-matrix.json" },
                requiredCapabilities,
                requiredDeviceTrust,
                businessLine.Level,
                businessLine.SurfaceMode,
        ...
        RuntimeActorContext actor)
    {
        ...
        if (businessLine is null)
        {
            return AdmissionKernelDecision.SearchVisible(
                definition,
                "blocked",
                "Search may show this compatibility result, but business line admission is unresolved and production remains blocked.",
                Array.Empty<string>(),
                Array.Empty<string>(),
                "unknown",
                "unknown",
        ...
        return AdmissionKernelDecision.SearchVisible(
            ...
            definition.Resolved
                ...
                : "Search result is visible through compatibility, but production confirm is blocked until Definition Registry resolves it.",
            requiredCapabilities,
            requiredDeviceTrust,
            businessLine.Level,
            businessLine.SurfaceMode,
    ...
    private static IReadOnlyList<string> RequiredCapabilitiesFor(WorkItemDefinitionResolution definition)
    {
        ...
        var legacyCard = definition.LegacyCardId;
        ...
        if (legacyCard.Contains("room", StringComparison.OrdinalIgnoreCase) ||
            legacyCard.Contains("bed", StringComparison.OrdinalIgnoreCase) ||
            legacyCard.Contains("rate", StringComparison.OrdinalIgnoreCase))
        {
    ...
    string AdmissionDecisionRef)
{
    public static AdmissionKernelDecision AllowedInternalPilot(
        ...
        new(
            ...
            reason,
            new[] { "docs/oma/current-admission-state.json" },
            requiredCapabilities,
            requiredDeviceTrust,
            businessLineState,
    ...
    private static string RefFor(WorkItemDefinitionResolution definition, string mode) =>
        $"admission:{definition.LegacyCardId}:{mode}:{OperationsHash.Short(definition.DefinitionId, definition.LegacyCardId, mode)[..12]}";
}
