...
internal static class WorkspaceSeedCatalog
{
    public static IReadOnlyList<WorkspaceSeed> All() => new[]
    {
        Workspace("W-STAY-RESOURCE", "stay", "T-ROOM-CREATE", "我要创建住宿资源", "Создать ресурсы проживания",
            "房间、床位、价格、准备度、阻断和释放在同一资源建档办理面完成。",
            "Комнаты, койки, тарифы, готовность, блокировка и освобождение вместе.",
            new[]
            {
                ...
                Card("roomRelease", "notStarted", "房间床位释放卡", "Освобождение", new[] { "releaseId", "roomId", "bedId" }, new[] { "房间", "床位", "释放范围", "恢复可售时间", "释放备注" }, new[] { "恢复可售床位数", "阻断恢复耗时" })
            },
            "先配置房间和床位，再配置价格、准备度、阻断和释放。",
            "Сначала комната и койка, затем тарифы, готовность, блокировка и освобождение."),
