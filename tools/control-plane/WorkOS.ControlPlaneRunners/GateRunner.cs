...
public static class GateRunner
{
    public static Task<GateResultEvidence> Run(RunnerOptions options)
    {
        ...
        var outputPath = options.Get("out", Path.Combine(".tmp", "oam", "gate-result.json"));
        ...
        var evidence = new GateResultEvidence(
            GateResultId: options.Get("id", "gate-oam-current-runner"),
            ReleaseId: releaseId,
            MrId: mrId,
            TenantId: options.Get("tenantId"),
            SliceId: options.Get("sliceId"),
            GateName: options.Get("gateName", "oam.current-control-plane"),
            GateType: options.Get("gateType", "automated"),
            Status: decision.Status,
            Severity: decision.Severity,
            CiRunId: ciRunId,
            AutomatedTestRefs: SplitRefs(options.Get("automated-test")).DefaultIfEmpty("scripts/oam/run-control-plane-checks.ps1").ToArray(),
