...
public static class BackupRestoreSmokeJob
{
    ...
    {
        var releaseId = options.Get("releaseId", "oam.current-backup-restore-smoke");
        ...
        var outputPath = options.Get("out", Path.Combine(".tmp", "oam", "backup-restore-smoke-invariant-checks.json"));
        var reportPath = options.Get("report-out", Path.Combine(".tmp", "oam", "backup-restore-smoke-report.json"));
        var invariantOutputPath = options.Get("restore-invariant-out", Path.Combine(".tmp", "oam", "backup-restore-after-restore-invariants.json"));
...
public sealed class PostgresBackupRestoreInvariantRunner : IBackupRestoreInvariantRunner
{
    ...
    {
        var definitionsPath = Path.Combine(".tmp", "oam", $"{context.ReportId}-after-restore-invariants.json");
