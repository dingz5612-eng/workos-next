...
public static class ShadowCompareRunner
{
    ...
    public static Task<ShadowCompareEvidence> Run(RunnerOptions options)
    {
        ...
        var outputPath = options.Get("out", Path.Combine(".tmp", "oam", "shadow-compare-report.json"));
        var configPath = options.Get("config", Path.Combine("docs", "oam", "shadow-compare.config.json"));
        var semanticRulesPath = options.Get("semantic-rules", Path.Combine("docs", "oam", "shadow-compare-semantic-rules.json"));
        ...
        string sourceMode = "real")
    {
        return new ShadowCompareEvidence(
            ShadowCompareReportId: $"scr-oam-current-{Sanitize(config.Name)}",
            ReleaseId: releaseId,
            TenantId: tenantId,
            SliceId: sliceId,
            CompareScope: compareScope ?? CompareScope(config),
            SourceBaselineRef: config.BaselineRef,
