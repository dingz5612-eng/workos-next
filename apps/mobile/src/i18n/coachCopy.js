export const coachCopy = {
  "zh-CN": {
    "learningCenter": "学习中心",
    "learningCenterBody": "你卡在哪一步？搜索业务词或点击闭环阶段，系统只展开这一阶段的字段、判断、证据、确认和下一步。",
    "scenarioCoach": "场景教练",
    "coachSearchPlaceholder": "搜索押金、报修、车辆到场、谁确认、缺什么材料",
    "coachPerspective": "学习视角",
    "coachNoMatch": "没有匹配的场景阶段，换一个业务词或动作词试试。",
    "coachAll": "全部",
    "coachHowTo": "怎么办理",
    "coachFields": "需要字段",
    "coachException": "异常阻断",
    "coachConfirm": "人工确认",
    "coachNext": "下一步",
    "coachAi": "AI 边界",
    "stageWhat": "这一步做什么",
    "stageFields": "需要填写什么",
    "stageJudgement": "系统会检查什么",
    "stageEvidence": "需要什么证据",
    "stageConfirm": "谁来确认",
    "stageAfter": "完成后进入什么状态",
    "stageNext": "下一步是什么",
    "openRelatedObject": "打开相关对象",
    "coachHowToUse": "搜索后会直接高亮并展开相关闭环阶段；不再把知识拆成脱离业务的结果列表。",
    "quickStart": "快速上手",
    "sceneLearning": "场景学习",
    "roleLearning": "角色与边界",
    "aiCanDo": "AI 可以解释、推荐、总结和生成草稿。",
    "aiCannotDo": "AI 不能自动确认、扣费、退款、核销或关闭终态。"
  },
  "ru-RU": {
    "learningCenter": "Учебный центр",
    "learningCenterBody": "Где вы застряли? Ищите бизнес-термин или выберите этап, система покажет поля, проверки, доказательства, подтверждение и следующий шаг.",
    "scenarioCoach": "Тренер сценариев",
    "coachSearchPlaceholder": "Депозит, заявка, прибытие авто, кто подтверждает, какие материалы",
    "coachPerspective": "Фокус обучения",
    "coachNoMatch": "Подходящий этап не найден. Попробуйте другой бизнес-термин или действие.",
    "coachAll": "Все",
    "coachHowTo": "Как выполнить",
    "coachFields": "Поля",
    "coachException": "Блокировки",
    "coachConfirm": "Подтверждение",
    "coachNext": "Следующий шаг",
    "coachAi": "Граница AI",
    "stageWhat": "Что сделать",
    "stageFields": "Какие поля нужны",
    "stageJudgement": "Что проверит система",
    "stageEvidence": "Какие доказательства нужны",
    "stageConfirm": "Кто подтверждает",
    "stageAfter": "Состояние после шага",
    "stageNext": "Следующий шаг",
    "openRelatedObject": "Открыть объект",
    "coachHowToUse": "Поиск сразу подсвечивает и раскрывает этап сценария; знания не отделены от бизнес-действия.",
    "quickStart": "Быстрый старт",
    "sceneLearning": "Сценарии",
    "roleLearning": "Роли и границы",
    "aiCanDo": "AI объясняет, рекомендует, резюмирует и готовит черновики.",
    "aiCannotDo": "AI не подтверждает, не списывает, не возвращает деньги и не закрывает финальные статусы."
  },
  "ky-KG": {
    "learningCenter": "Окуу борбору",
    "learningCenterBody": "Кайсы кадамда токтодуңуз? Бизнес сөздү издеңиз же этапты тандаңыз, система талааларды, текшерүүлөрдү, далилдерди, тастыктоону жана кийинки кадамды көрсөтөт.",
    "scenarioCoach": "Сценарий машыктыруучусу",
    "coachSearchPlaceholder": "Депозит, билдирме, ким тастыктайт, кандай материал керек",
    "coachPerspective": "Окуу көз карашы",
    "coachNoMatch": "Дал келген этап табылган жок. Башка бизнес сөз же аракет менен издеп көрүңүз.",
    "coachAll": "Баары",
    "coachHowTo": "Кантип аткарылат",
    "coachFields": "Талаалар",
    "coachException": "Тоскоолдуктар",
    "coachConfirm": "Тастыктоо",
    "coachNext": "Кийинки кадам",
    "coachAi": "AI чеги",
    "stageWhat": "Бул кадамда эмне кылынат",
    "stageFields": "Кайсы талаалар керек",
    "stageJudgement": "Система эмнени текшерет",
    "stageEvidence": "Кайсы далил керек",
    "stageConfirm": "Ким тастыктайт",
    "stageAfter": "Кадамдан кийинки абал",
    "stageNext": "Кийинки кадам",
    "openRelatedObject": "Объектти ачуу",
    "coachHowToUse": "Издөө тиешелүү этапты дароо ачат; билим бизнес аракеттен бөлүнбөйт.",
    "quickStart": "Тез баштоо",
    "sceneLearning": "Сценарийлер",
    "roleLearning": "Роль жана чек",
    "aiCanDo": "AI түшүндүрөт, сунуштайт, жыйынтыктайт жана черновик даярдайт.",
    "aiCannotDo": "AI автоматтык тастыктабайт, кармабайт, кайтарбайт же акыркы статусту жаппайт."
  }
};
