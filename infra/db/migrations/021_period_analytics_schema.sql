...
do $$
begin
    if to_regclass('period_metric_snapshots') is not null then
        drop trigger if exists trg_period_metric_snapshots_append_only_after_close on period_metric_snapshots;
    end if;

    if to_regclass('period_finance_snapshots') is not null then
        drop trigger if exists trg_period_finance_snapshots_append_only_after_close on period_finance_snapshots;
    end if;

    if to_regclass('period_operation_snapshots') is not null then
        drop trigger if exists trg_period_operation_snapshots_append_only_after_close on period_operation_snapshots;
    end if;

    if to_regclass('period_late_adjustments') is not null then
        drop trigger if exists trg_period_late_adjustments_forbid_update on period_late_adjustments;
        drop trigger if exists trg_period_late_adjustments_forbid_delete on period_late_adjustments;
    end if;
end $$;
...
begin
    if not exists (
        select 1
        from pg_constraint
        where conname = 'uq_period_reviews_period_review_id'
          and conrelid = 'period_reviews'::regclass
    ) then
        alter table period_reviews
...
begin
    alter table period_reviews
        add constraint ck_period_reviews_status
        check (status in (
            'open',
            'scope_confirmed',
            'metrics_reviewed',
...
begin
    if not exists (
        ...
        where conname = 'uq_period_metric_snapshots_snapshot_id'
...
begin
    if not exists (
        select 1 from pg_constraint
        where conname = 'uq_period_finance_snapshots_snapshot_id'
...
begin
    if not exists (
        select 1 from pg_constraint
        where conname = 'ck_period_finance_snapshots_expense_status'
          and conrelid = 'period_finance_snapshots'::regclass
    ) then
        alter table period_finance_snapshots
...
begin
    if not exists (
        select 1 from pg_constraint
        where conname = 'ck_period_action_plans_owner_due_priority'
          and conrelid = 'period_action_plans'::regclass
    ) then
        alter table period_action_plans
...
begin
    if not exists (
        select 1 from pg_constraint
        where conname = 'uq_period_late_adjustments_late_adjustment_id'
          and conrelid = 'period_late_adjustments'::regclass
    ) then
        alter table period_late_adjustments
...
begin
    if not exists (
        select 1 from pg_constraint
        where conname = 'ck_period_late_adjustments_body_object'
          and conrelid = 'period_late_adjustments'::regclass
    ) then
        alter table period_late_adjustments
