...
public sealed class DormitoryRuntimeAdmissionGuardTests
{
    ...
    {
        using var contract = DormitoryRuntimeReplayHarness.ReadJson("docs/go-live/dormitory/runtime-replay-contract.yml");
        var boundary = contract.RootElement.GetProperty("runtimeBoundary");

        Assert.AreEqual("L1 Internal Pilot", boundary.GetProperty("dormitoryStatus").GetString());
        Assert.IsFalse(boundary.GetProperty("dormitoryProductionAllowed").GetBoolean());
        Assert.AreEqual("L0 Contract Preview", boundary.GetProperty("repairStatus").GetString());
        Assert.AreEqual("L0 Contract Preview", boundary.GetProperty("partsStatus").GetString());
        Assert.AreEqual("L0 Contract Preview", boundary.GetProperty("hrStatus").GetString());
