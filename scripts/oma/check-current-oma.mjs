...
const requiredJson = [
  ...
  "modules/accommodation/oma-module.manifest.json",
  "modules/finance-gate/oma-module.manifest.json",
  "modules/identity/oma-module.manifest.json",
  "modules/maintenance/oma-module.manifest.json"
];
...
requireFile("docs/oma/current-architecture.md");
...
for (const moduleName of ["accommodation", "finance-gate", "identity", "maintenance"]) {
  const manifest = readJson(`modules/${moduleName}/oma-module.manifest.json`);
...
if (exists(".github/workflows/oma_control_plane.yml")) {
  violations.push(v("legacy_workflow_present", "旧 OMA current workflow 不得存在。"));
}

for (const file of existingFilesUnder("artifacts")) {
  const normalized = slash(file);
  if (!normalized.startsWith("artifacts/oma/")) {
    violations.push(v("legacy_artifact_present", `非 OMA artifact 不得保留：${normalized}`));
  }
}

for (const file of existingFilesUnder("apps/mobile/artifacts")) {
...
if (violations.length) {
  ...
  throw new Error(`OMA purity check failed: ${violations.length} violation(s).`);
}

console.log("OMA purity check: PASS");
...
function checkGithubText(file) {
  ...
  for (const pattern of forbidden) {
    if (pattern.test(text)) {
      violations.push(v("github_legacy_term", `${file} 含旧阶段语义 ${pattern}.`));
    }
  }
}
...
function checkGlobalRetiredTerms() {
  ...
  for (const file of filesUnder(".")) {
    ...
    for (const pattern of forbidden) {
      if (pattern.test(text)) {
        violations.push(v("global_legacy_term", `${normalized} 含旧阶段语义 ${pattern}.`));
      }
    }
  }
}
...
function checkDirectory(dir, allowed) {
  ...
  for (const name of actual) {
    if (!allowed.includes(name)) {
      violations.push(v("directory_not_allowed", `${dir}/${name} 不在 OMA 允许目录内。`));
    }
  }
  for (const name of allowed) {
    ...
    if (!actual.includes(name)) {
      violations.push(v("directory_required_missing", `${dir}/${name} 是当前 OMA 必需目录。`));
    }
  }
}
...
function shouldScanText(file) {
  ...
  if (file === "scripts/oma/check-current-oma.mjs") return false;
  if (file.includes("/node_modules/")) return false;
  if (file.includes("/bin/") || file.includes("/obj/")) return false;
  if (file.includes("/TestResults/")) return false;
  if (file.startsWith("artifacts/oma/checks/")) return false;
  if (file.startsWith("artifacts/oma/test-results/")) return false;
