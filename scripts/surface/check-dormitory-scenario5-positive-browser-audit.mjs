...
const reportRelPath = "artifacts/oam/evidence/dormitory-scenario5-reservation-and-inventory-hold-positive-browser/scenario5-positive-browser-report.json";
const resultRelPath = "artifacts/oam/checks/dormitory-scenario5-positive-browser-result.json";
...
if (report.authorityId !== "Dormitory.Scenario5.ReservationAndInventoryHold") failures.push("positive report must bind scenario5 authority.");
if (report.nameZh !== "预订与库存锁定") failures.push("positive report must show business name 预订与库存锁定.");
...
for (const name of ["进入预订确认", "复核可订资源", "锁定库存", "确认预订信息", "生成预订", "预订结果与下游准备"]) {
...
if (!JSON.stringify(report).includes("锁定截止时间") ||
  !JSON.stringify(report).includes("系统生成预订号") ||
  !JSON.stringify(report).includes("搜索结果只读跳转") ||
  !JSON.stringify(report).includes("我的只放草稿") ||
  !JSON.stringify(report).includes("后续仍需重新核验")) {
  failures.push("positive report must prove hold deadline, system reservation number, readonly search, Mine duties, and downstream recheck.");
}
if ((stepsContract.steps ?? []).length !== 6) failures.push("scenario5 generated steps contract must expose six business actions.");

const result = {
  version: "oam.dormitory-scenario5-positive-browser-check.v1",
...
if (result.status !== "PASS") {
  console.error("Dormitory scenario5 positive browser audit check: FAIL");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log(`Dormitory scenario5 positive browser audit check: PASS (${result.positiveBrowserAuditDigest})`);
