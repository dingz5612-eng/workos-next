...
const reportRelPath = "artifacts/oam/evidence/dormitory-scenario3-product-and-pricing-positive-browser/scenario3-positive-browser-report.json";
const resultRelPath = "artifacts/oam/checks/dormitory-scenario3-positive-browser-result.json";
...
if (report.authorityId !== "Dormitory.Scenario3.ProductAndPricing") failures.push("positive report must bind scenario3 authority.");
if (report.nameZh !== "住宿商品与价格") failures.push("positive report must show business name 住宿商品与价格.");
...
if (!Array.isArray(report.steps) || report.steps.length < 10) failures.push("positive report must include at least 10 positive screenshots/steps.");
...
for (const name of ["选择可运营房源", "定义住宿商品", "配置价格方案", "配置适用日期和规则", "审核与生效确认", "价格维护"]) {
...
const result = {
  version: "oam.dormitory-scenario3-positive-browser-check.v1",
...
if (result.status !== "PASS") {
  console.error("Dormitory scenario3 positive browser audit check: FAIL");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log(`Dormitory scenario3 positive browser audit check: PASS (${result.positiveBrowserAuditDigest})`);
