...
const reportRelPath = "artifacts/oam/evidence/dormitory-scenario6-payment-deposit-and-guarantee-negative-browser/scenario6-negative-browser-report.json";
const resultRelPath = "artifacts/oam/checks/dormitory-scenario6-negative-browser-result.json";
...
if (report.authorityId !== "Dormitory.Scenario6.PaymentDepositAndGuarantee") failures.push("negative report must bind scenario6 authority.");
if (report.nameZh !== "收款、押金与担保") failures.push("negative report must show business name 收款、押金与担保.");
...
if (financeGate.financeBoundaryRule?.financeGateRequired !== true ||
  ...
  failures.push("finance-gate generated contract must enforce scenario6 negative boundary.");
}

const result = {
  version: "oam.dormitory-scenario6-negative-browser-check.v1",
...
if (result.status !== "PASS") {
  console.error("Dormitory scenario6 negative browser audit check: FAIL");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log(`Dormitory scenario6 negative browser audit check: PASS (${result.negativeBrowserAuditDigest})`);
