...
const reportRelPath = "artifacts/oam/evidence/dormitory-scenario11-housekeeping-maintenance-outofservice-negative-browser/scenario11-negative-browser-report.json";
const resultRelPath = "artifacts/oam/checks/dormitory-scenario11-negative-browser-result.json";
...
if (report.authorityId !== "Dormitory.Scenario11.HousekeepingMaintenanceOutOfService") failures.push("negative report must bind scenario11 authority.");
if (report.nameZh !== "房务、维修与停售协同") failures.push("negative report must show business name 房务、维修与停售协同.");
...
if (!Array.isArray(report.scenarios) || report.scenarios.length !== 12) failures.push("negative report must include exactly 12 failure scenarios.");
...
const result = {
  version: "oam.dormitory-scenario11-negative-browser-check.v1",
...
if (result.status !== "PASS") {
  console.error("Dormitory scenario11 negative browser audit check: FAIL");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log(`Dormitory scenario11 negative browser audit check: PASS (${result.negativeBrowserAuditDigest})`);
