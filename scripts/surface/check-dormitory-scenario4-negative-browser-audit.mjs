...
const reportRelPath = "artifacts/oam/evidence/dormitory-scenario4-inquiry-and-quote-negative-browser/scenario4-negative-browser-report.json";
const resultRelPath = "artifacts/oam/checks/dormitory-scenario4-negative-browser-result.json";
...
if (report.authorityId !== "Dormitory.Scenario4.InquiryAndQuote") failures.push("negative report must bind scenario4 authority.");
if (report.nameZh !== "询价与报价") failures.push("negative report must show business name 询价与报价.");
...
if (!JSON.stringify(report.scenarios ?? []).includes("搜索结果只读") ||
  !JSON.stringify(report.scenarios ?? []).includes("已发送报价不能原地编辑") ||
  !JSON.stringify(report.scenarios ?? []).includes("未写入任何业务事实") ||
  !JSON.stringify(report.scenarios ?? []).includes("发起转预订准备，由下一场景复核")) {
  failures.push("negative report must prove search readonly, issued quote inline edit blocked, no side effects, and downstream handoff only.");
}

const result = {
  version: "oam.dormitory-scenario4-negative-browser-check.v1",
...
if (result.status !== "PASS") {
  console.error("Dormitory scenario4 negative browser audit check: FAIL");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log(`Dormitory scenario4 negative browser audit check: PASS (${result.negativeBrowserAuditDigest})`);
