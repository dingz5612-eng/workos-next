...
const reportRelPath = "artifacts/oam/evidence/dormitory-scenario10-cancel-noshow-refund-negative-browser/scenario10-negative-browser-report.json";
const resultRelPath = "artifacts/oam/checks/dormitory-scenario10-negative-browser-result.json";
...
if (report.authorityId !== "Dormitory.Scenario10.CancelNoShowRefund") failures.push("negative report must bind scenario10 authority.");
if (report.nameZh !== "取消、未到店与退款处理") failures.push("negative report must show business name 取消、未到店与退款处理.");
...
if (!Array.isArray(report.scenarios) || report.scenarios.length !== 15) failures.push("negative report must include exactly 15 failure scenarios.");
...
if (!JSON.stringify(report.scenarios ?? []).includes("未写入任何业务结果") ||
  ...
  !JSON.stringify(report.scenarios ?? []).includes("转负责人复核") ||
  !JSON.stringify(report.scenarios ?? []).includes("本预订绑定资源和日期范围")) {
  failures.push("negative report must prove no side effects, readonly search, finance confirmation handoff, dispute review, and bounded inventory release.");
}
...
const result = {
  version: "oam.dormitory-scenario10-negative-browser-check.v1",
...
if (result.status !== "PASS") {
  console.error("Dormitory scenario10 negative browser audit check: FAIL");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log(`Dormitory scenario10 negative browser audit check: PASS (${result.negativeBrowserAuditDigest})`);
