...
const reportRelPath = "artifacts/oam/evidence/dormitory-scenario8-in-stay-management-negative-browser/scenario8-negative-browser-report.json";
const resultRelPath = "artifacts/oam/checks/dormitory-scenario8-negative-browser-result.json";
...
if (report.authorityId !== "Dormitory.Scenario8.InStayManagement") failures.push("negative report must bind scenario8 authority.");
if (report.nameZh !== "在住管理") failures.push("negative report must show business name 在住管理.");
...
if (!JSON.stringify(report.scenarios ?? []).includes("未写入任何业务事实") ||
  ...
  !JSON.stringify(report.scenarios ?? []).includes("不能发放或恢复凭证") ||
  !JSON.stringify(report.scenarios ?? []).includes("进入退房结算场景处理后续事项")) {
  failures.push("negative report must prove no side effects, readonly search, credential block, and downstream checkout separation.");
}

const result = {
  version: "oam.dormitory-scenario8-negative-browser-check.v1",
...
if (result.status !== "PASS") {
  console.error("Dormitory scenario8 negative browser audit check: FAIL");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log(`Dormitory scenario8 negative browser audit check: PASS (${result.negativeBrowserAuditDigest})`);
