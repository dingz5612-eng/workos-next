...
const reportRelPath = "artifacts/oam/evidence/dormitory-scenario2-resource-operation-status-positive-browser/scenario2-positive-browser-report.json";
const resultRelPath = "artifacts/oam/checks/dormitory-scenario2-positive-browser-result.json";
...
if (report.authorityId !== "Dormitory.Scenario2.ResourceOperationStatus") failures.push("positive report must bind scenario2 authority.");
if (report.nameZh !== "房源运营就绪与状态维护") failures.push("positive report must show business name 房源运营就绪与状态维护.");
...
if (!Array.isArray(report.steps) || report.steps.length < 11) failures.push("positive report must include at least 11 positive screenshots/steps.");
...
for (const name of ["选择已基础就绪房源", "运营检查", "设置运营状态", "影响确认", "日常状态维护", "恢复运营"]) {
...
for (const expected of contract.upstream?.requiredReadonlyInputs ?? []) {
  if (!JSON.stringify(report.steps ?? []).includes(expected)) failures.push(`positive report missing upstream readonly input: ${expected}`);
}
for (const expected of contract.downstream?.handoffOutputs ?? []) {
  if (!JSON.stringify(report.steps ?? []).includes(expected)) failures.push(`positive report missing downstream handoff output: ${expected}`);
}
...
if (!JSON.stringify(report).includes("搜索结果：只读跳转") ||
  !JSON.stringify(report).includes("我的只放草稿")) {
  failures.push("positive report must prove Search is readonly and Mine has personal duties only.");
}

const result = {
  version: "oam.dormitory-scenario2-positive-browser-check.v1",
...
if (result.status !== "PASS") {
  console.error("Dormitory scenario2 positive browser audit check: FAIL");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log(`Dormitory scenario2 positive browser audit check: PASS (${result.positiveBrowserAuditDigest})`);
