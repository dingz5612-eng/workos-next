...
const reportRelPath = "artifacts/oam/evidence/dormitory-scenario5-reservation-and-inventory-hold-negative-browser/scenario5-negative-browser-report.json";
const resultRelPath = "artifacts/oam/checks/dormitory-scenario5-negative-browser-result.json";
...
if (report.authorityId !== "Dormitory.Scenario5.ReservationAndInventoryHold") failures.push("negative report must bind scenario5 authority.");
if (report.nameZh !== "预订与库存锁定") failures.push("negative report must show business name 预订与库存锁定.");
...
if (!JSON.stringify(report.scenarios ?? []).includes("未写入任何业务事实") ||
  ...
  !JSON.stringify(report.scenarios ?? []).includes("库存锁定已过期") ||
  !JSON.stringify(report.scenarios ?? []).includes("预订号由系统生成，不能手工填写")) {
  failures.push("negative report must prove no side effects, readonly search, expired hold, and system generated reservation number.");
}

const result = {
  version: "oam.dormitory-scenario5-negative-browser-check.v1",
...
if (result.status !== "PASS") {
  console.error("Dormitory scenario5 negative browser audit check: FAIL");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log(`Dormitory scenario5 negative browser audit check: PASS (${result.negativeBrowserAuditDigest})`);
