...
const reportRelPath = "artifacts/oam/evidence/dormitory-scenario13-reporting-audit-review-negative-browser/scenario13-negative-browser-report.json";
const resultRelPath = "artifacts/oam/checks/dormitory-scenario13-negative-browser-result.json";
...
if (report.authorityId !== "Dormitory.Scenario13.ReportingAuditReview") failures.push("negative report must bind scenario13 authority.");
if (report.nameZh !== "经营报表、审计与复盘") failures.push("negative report must show business name 经营报表、审计与复盘.");
...
if (!Array.isArray(report.scenarios) || report.scenarios.length !== 11) failures.push("negative report must include exactly 11 failure scenarios.");
...
for (const code of [
  ...
  "missing_lineage_envelope",
  "stale_freshness_envelope",
  "ui_state_metric_forbidden",
  "non_finance_gate_truth_forbidden",
  "audit_direct_source_fix_forbidden",
  "published_report_inline_edit_forbidden",
  "forged_internal_reference",
  "readonly_search_write_attempt",
  "duplicate_report_publish",
  "concurrent_report_publish_conflict"
]) {
...
const result = {
  version: "oam.dormitory-scenario13-negative-browser-check.v1",
...
if (result.status !== "PASS") {
  console.error("Dormitory scenario13 negative browser audit check: FAIL");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log(`Dormitory scenario13 negative browser audit check: PASS (${result.negativeBrowserAuditDigest})`);
