...
const mobileBusinessDisplayLanguagePath = "apps/mobile/src/generated/oam/business-display-language.generated.js";
...
const optionSets = {
  ...
  bedEnabledStatus: [
    { value: "enabled", label: { "zh-CN": "启用", "ru-RU": "Включено", "ky-KG": "Иштейт" } },
    { value: "disabled", label: { "zh-CN": "停用", "ru-RU": "Отключено", "ky-KG": "Токтотулган" } }
  ],
  bedTypeBatchSetting: [
    { value: "bunk_pair", label: { "zh-CN": "按上下铺批量生成", "ru-RU": "Пакетно как двухъярусные", "ky-KG": "Эки кабат катары топтом түзүү" } },
...
writeText(mobileBusinessDisplayLanguagePath, businessDisplayLanguageModule());
...
function buildSteps() {
  ...
  return (workitems.workItems ?? []).map((item, index, all) => {
    ...
    const fields = stepControls
      .filter((control) => isStepVisibleControl(control))
...
function businessDisplayLanguageModule() {
  const replacements = visibleBusinessCopyContract.displayTermReplacementsZh ?? [];
  const ruReplacements = visibleBusinessCopyContract.displayTermReplacementsRu ?? [];
  const kyReplacements = visibleBusinessCopyContract.displayTermReplacementsKy ?? [];
  const riskLabels = visibleBusinessCopyContract.riskLabelReplacementsZh ?? {};
  return [
    "// Generated by scripts/oam/compile-current-capability.mjs from docs/oam/visible-business-copy-contract.json.",
    "// Do not edit manually.",
    `export const zhBusinessTermReplacements = ${JSON.stringify(replacements, null, 2)};`,
    `export const ruBusinessTermReplacements = ${JSON.stringify(ruReplacements, null, 2)};`,
    `export const kyBusinessTermReplacements = ${JSON.stringify(kyReplacements, null, 2)};`,
    `export const zhRiskLabelReplacements = ${JSON.stringify(riskLabels, null, 2)};`,
    "export const businessDisplayLanguageContract = {",
    `  version: ${JSON.stringify(visibleBusinessCopyContract.version)},`,
    `  source: ${JSON.stringify(visibleBusinessCopyContractPath)},`,
    `  finalGoNoGo: ${JSON.stringify(visibleBusinessCopyContract.finalGoNoGo)}`,
    "};",
    ""
  ].join("\n");
}
...
function buildMobileSurfaceControls() {
  ...
  for (const control of scenario1AuthoritySurfaceControls()) {
    addSurfaceControl(controlsByKey, control);
  }
  for (const control of surfaceModel.controls ?? []) {
    const normalized = normalizeLegacySurfaceControl(control);
    const keepNormalized = shouldKeepLegacySurfaceControl(normalized);
    if (keepNormalized) {
      addSurfaceControl(controlsByKey, normalized, { replace: false });
    }
    const sourceAlias = sourceAliasReadonlyControl(control, normalized, keepNormalized);
    if (sourceAlias) {
      addSurfaceControl(controlsByKey, sourceAlias, { replace: false });
    }
  }
  for (const item of workitems.workItems ?? []) {
    for (const spec of systemDerivedSpecsForWorkItem(item.workItemType)) {
      ...
      if (controlsByKey.has(key)) continue;
      addSurfaceControl(controlsByKey, {
        workItemType: item.workItemType,
        definitionId: item.definitionId,
  ...
  return Array.from(controlsByKey.values()).map(enrichSurfaceControl);
}

function addSurfaceControl(controlsByKey, control, options = {}) {
  if (!control?.workItemType || !control?.fieldId) return;
  const key = `${control.workItemType}:${control.fieldId}`;
  if (options.replace === false && controlsByKey.has(key)) return;
  controlsByKey.set(key, control);
}

function isStepVisibleControl(control) {
  if (["clientSubmitted", "selectedStableRef", "systemDerived"].includes(control.classification)) return true;
  return control.classification === "contextReadonly" && control.fieldId === "buildingContextRef";
}

function normalizeLegacySurfaceControl(control) {
  const fieldId = canonicalScenario1SurfaceFieldId(control.fieldId);
  return {
    ...control,
    fieldId,
    controlType: controlTypeForField(fieldId, control.controlType),
    required: control.required ?? (control.classification === "clientSubmitted")
  };
}

function shouldKeepLegacySurfaceControl(control) {
  const step = (scenario1StepsFields.steps ?? []).find((item) => item.commandId === control.workItemType);
  if (!step) return true;
  if (control.classification !== "clientSubmitted") return true;
  return scenario1OrderedUserFields(step).includes(canonicalScenario1SurfaceFieldId(control.fieldId));
}

function sourceAliasReadonlyControl(control, normalized, keepNormalized) {
  const originalKey = `${control.workItemType}:${control.fieldId}`;
  ...
  if (!sourceAliasFor) return null;
  if (keepNormalized && originalKey === normalizedKey) return null;
  return {
    ...control,
    fieldId: control.fieldId,
    classification: "sourceAliasReadonly",
    controlType: "hidden",
    required: false,
    fallbackAllowed: false,
    forbiddenFallbackControls: ["text", "textarea", "select", "combobox"],
...
function scenario1AuthoritySurfaceControls() {
  const controls = [];
  for (const step of scenario1StepsFields.steps ?? []) {
    const item = (workitems.workItems ?? []).find((candidate) => candidate.workItemType === step.commandId);
    if (!item) continue;
    if (step.commandId === "Dorm.BedSetupConfirm") {
      controls.push(authorityControl(item, "bedCount", "systemDerived", { required: false, controlType: "readonly" }));
      controls.push(authorityControl(item, "bedLabels", "systemDerived", { required: true, controlType: "readonly" }));
    }
    for (const fieldId of scenario1OrderedUserFields(step)) {
      const canonical = canonicalScenario1SurfaceFieldId(fieldId);
      const binding = fieldBindingFor(canonical);
      const selectedField = (step.userSelectedFields ?? []).includes(fieldId);
      const classification = selectedField && binding?.classification === "contextReadonly" ? "contextReadonly" : "clientSubmitted";
      controls.push(authorityControl(item, canonical, classification, {
        required: classification === "contextReadonly" ? false : requiredScenario1UserField(canonical),
        controlType: classification === "contextReadonly" ? "readonly" : controlTypeForField(canonical)
      }));
    }
  }
  return controls;
...
function scenario1OrderedUserFields(step) {
  const userFields = [
    ...(step.userFilledFields ?? []),
    ...(step.userSelectedFields ?? [])
  ].map(canonicalScenario1SurfaceFieldId);
  const userFieldSet = new Set(userFields);
  const orderedByUserSees = (step.userSees ?? [])
    .map(userVisibleLabelToFieldId)
    .filter((fieldId) => fieldId && userFieldSet.has(fieldId));
  return [
    ...orderedByUserSees,
    ...userFields.filter((fieldId) => !orderedByUserSees.includes(fieldId))
  ];
}

function userVisibleLabelToFieldId(label = "") {
  ...
  const mapping = {
    ...
    "安全基础检查结果": "safetyBasicCheckResult"
  };
  return mapping[normalized] || "";
}

function authorityControl(item, fieldId, classification, overrides = {}) {
  return {
    workItemType: item.workItemType,
    definitionId: item.definitionId,
    fieldId,
    classification,
    controlType: overrides.controlType ?? controlTypeForField(fieldId),
    required: overrides.required ?? classification === "clientSubmitted",
    fallbackAllowed: false,
    forbiddenFallbackControls: forbiddenFallbackControlsForField(fieldId, classification),
    source: "scenario1-steps-fields.generated.json"
  };
}
...
function fieldBindingFor(fieldId) {
  return (fieldBindings.fieldBindings ?? []).find((binding) => binding.fieldId === fieldId);
}

function requiredScenario1UserField(fieldId) {
  return !["roomRemark", "bedRemark", "specialNotes", "basicReadinessRemark"].includes(fieldId);
}
...
function enrichSurfaceControl(control) {
  ...
  const required = control.required ?? role.userSubmitted;
  return {
    ...control,
...
function fieldProjectionForControl(control, workItemType) {
  ...
  return {
    ...
    hiddenSubmitOnly: role.surface === "hidden-submit-only",
    userSubmitted: role.userSubmitted,
    required: control.required ?? role.userSubmitted,
    displayValueSource: role.displayValueSource,
    submitValueSource: role.submitValueSource,
    branchOutput: branchOutputForField(control.fieldId),
    branchRequiredFields: branchRequiredFieldsForField(control.fieldId),
...
function fieldRole(workItemType, fieldId, classification) {
  ...
  if (classification !== "clientSubmitted" && branchOutputFieldIds().includes(fieldId)) {
    return {
  ...
  if (classification === "contextReadonly") {
    return {
      fieldCategory: "systemDerivedFields",
      userSubmitted: false,
      readonly: true,
  ...
  if (classification === "systemGenerated") {
    return {
      fieldCategory: "systemDerivedFields",
      userSubmitted: false,
      readonly: true,
      surface: "hidden-submit-only",
      controlType: "hidden",
      displayValueSource: "generated runtime value",
      submitValueSource: "generated runtime value",
      source: "system generated"
    };
  }
  if (classification === "sourceAliasReadonly") {
    return {
      fieldCategory: "systemDerivedFields",
      userSubmitted: false,
      readonly: true,
      surface: "hidden-submit-only",
      controlType: "hidden",
      displayValueSource: "generated current-field alias display value",
      submitValueSource: "generated current-field alias submit value",
      source: "generated source-control alias"
    };
  }
  return {
...
function defaultValueForField(fieldId) {
  ...
  if (fieldId === "bedEnabledStatus") return "enabled";
  if (fieldId === "bedTypeBatchSetting") return optionSetDefaults.bunkType.multiBed;
...
function buildFieldCategories(currentSteps) {
  ...
  return {
    requiredCoreFields: entries.filter((entry) => entry.userSubmitted && entry.required !== false),
    optionalBusinessFields: entries.filter((entry) => entry.userSubmitted && entry.required === false),
    systemDerivedFields: entries.filter((entry) => !entry.userSubmitted && entry.readonly),
...
function capabilityBusinessUi() {
  return {
    ...
    runtimeTestOnlySubmitLabel: { "zh-CN": "提交", "ru-RU": "Отправить", "ky-KG": "Тапшыруу" },
    businessLandingSubmitLabels: {
      "Dorm.RoomSetupConfirm": { "zh-CN": "提交房间信息", "ru-RU": "Отправить комнату", "ky-KG": "Бөлмө маалыматын тапшыруу" },
      "Dorm.BedSetupConfirm": { "zh-CN": "提交床位信息", "ru-RU": "Отправить койки", "ky-KG": "Койка маалыматын тапшыруу" },
      "Dorm.ResourceReadinessConfirm": { "zh-CN": "提交基础检查", "ru-RU": "Отправить проверку", "ky-KG": "Негизги текшерүүнү тапшыруу" }
    },
...
function titleFor(workItemType, index, total) {
  const titles = {
    "Dorm.RoomSetupConfirm": {
      "zh-CN": `${index}/${total} 填写房间信息`,
      "ru-RU": `${index}/${total} Данные комнаты`,
      "ky-KG": `${index}/${total} Бөлмө маалыматы`
    },
    "Dorm.BedSetupConfirm": {
      "zh-CN": `${index}/${total} 确认床位信息`,
      "ru-RU": `${index}/${total} Данные коек`,
      "ky-KG": `${index}/${total} Койка маалыматы`
    },
    "Dorm.ResourceReadinessConfirm": {
      "zh-CN": `${index}/${total} 完成基础检查`,
      "ru-RU": `${index}/${total} Базовая проверка`,
      "ky-KG": `${index}/${total} Негизги текшерүү`
    }
  };
...
function labelFor(fieldId) {
  const labels = {
    ...
    floor: ["楼层", "Этаж", "Кабат"],
    bedCount: ["床位数", "Количество коек", "Койка саны"],
    ...
    specialNotes: ["特殊说明", "Особые заметки", "Өзгөчө эскертүү"],
    bedEnabledStatus: ["床位启用状态", "Статус койки", "Койка абалы"],
    bedTypeBatchSetting: ["床型批量设置", "Пакетная настройка типа", "Койка түрүн топтом орнотуу"],
    bedId: ["床位", "Койка", "Койка"],
    readinessState: ["基础检查结论", "Статус проверки", "Текшерүү жыйынтыгы"],
    basicCheckResult: ["基础检查结果", "Результат базовой проверки", "Негизги текшерүү жыйынтыгы"],
    cleaningBasicCheckResult: ["保洁检查结果", "Проверка уборки", "Тазалык текшерүүсү"],
    facilityBasicCheckResult: ["设施检查结果", "Проверка оборудования", "Жабдуу текшерүүсү"],
    safetyBasicCheckResult: ["安全检查结果", "Проверка безопасности", "Коопсуздук текшерүүсү"],
    basicReadinessRemark: ["检查备注", "Комментарий", "Эскертүү"],
    supplementEvidenceRefs: ["补充材料", "Дополнительные материалы", "Кошумча материал"],
