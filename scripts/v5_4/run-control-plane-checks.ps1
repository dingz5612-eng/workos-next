...
node scripts/v5_4/gate-runner.mjs --formal-release-gate=true --rollback=docs/v5.4/rollback/mr-00-rollback-instruction.json --id=gate-mr-00-control-plane-bootstrap --releaseId=$releaseId --mrId=$mrId --ciRunId=$ciRunId --require-business-signoff=false --invariant=.tmp/v5_4/invariant-check.json --shadow=.tmp/v5_4/shadow-compare-report.json --out=.tmp/v5_4/gate-result.json
