...
const remoteVerified = (cli.value("remoteVerified") ?? process.env.V55_REMOTE_VERIFIED ?? "false")
  .toLowerCase() === "true";
const ciRunId = cli.value("ciRunId") ?? process.env.V55_CI_RUN_ID ?? "26685133709";
const v54RunId = cli.value("v54RunId") ?? process.env.V55_V54_CONTROL_PLANE_RUN_ID ?? "26685133695";
const commitSha = cli.value("commitSha") ?? process.env.V55_COMMIT_SHA ?? "";
const checks = [
...
const gate = {
  ...
  next_batch: failed.length === 0 && remoteVerified ? "ALLOWED" : failed.length === 0 ? "ALLOWED_AFTER_CURRENT_REMOTE_GREEN" : "BLOCKED",
  ...
  evidence: {
    ...
    latest_known_ci_run_id: ciRunId,
    latest_known_v54_control_plane_guards_run_id: v54RunId,
...
function writeDoc(gateResult) {
  ...
  const nextBatch = gateResult.next_batch;
...
The current V5.5 rules batch remote evidence is green:

- Commit: \`${gateResult.evidence.current_commit_sha || "not recorded"}\`
...
- P1: none after current pushed commit was revalidated remotely.
