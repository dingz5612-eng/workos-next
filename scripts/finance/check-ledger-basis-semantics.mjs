...
writeReport("artifacts/oma/checks/ledger-semantic-result.json", checkName, violations, scannedFiles);
...
function validateBasisTypes() {
  ...
  for (const basisType of ["deposit_receipt", "payment_receipt", "refund_deposit", "deposit_deduction", "ledger_correction_apply"]) {
    ...
    if (!contract) violations.push(violation("oma.finance.basis_missing", "docs/finance/ledger-basis-type-contract.yml", `缺少 basisType: ${basisType}.`, { basisType }));
    if (!rule) violations.push(violation("oma.finance.semantic_rule_missing", "docs/business/finance/ledger-semantic-rules.yml", `缺少语义规则: ${basisType}.`, { basisType }));
    if (!ledgerSource.includes(`case "${basisType}"`)) {
      violations.push(violation("oma.finance.runtime_basis_missing", "services/core-api/WorkOS.Api/Runtime/LedgerSemanticRules.cs", `LedgerSemanticRules 必须覆盖 ${basisType}.`, { basisType }));
    }
  }
  ...
  if (deposit?.credit?.accountType !== "liability" || !(deposit?.forbiddenAccountTypes ?? []).includes("revenue")) {
    violations.push(violation("oma.finance.deposit_semantics_wrong", "docs/finance/ledger-basis-type-contract.yml", "deposit_receipt 必须 credit liability 且禁止 revenue。"));
  }
  ...
  if (payment?.credit?.accountType !== "receivable" || !(payment?.forbiddenAccountTypes ?? []).includes("liability.deposit")) {
    violations.push(violation("oma.finance.payment_semantics_wrong", "docs/finance/ledger-basis-type-contract.yml", "payment_receipt 必须 credit receivable 且禁止 liability.deposit。"));
  }
  ...
  if (refund?.debit?.accountId !== "originalDepositAccountId" || !(refund?.amountRules ?? []).includes("refundAmount <= availableLiability")) {
    violations.push(violation("oma.finance.refund_semantics_wrong", "docs/finance/ledger-basis-type-contract.yml", "refund_deposit 必须引用原押金账户且限制可退负债。"));
  }
  ...
  for (const failure of [
    ...
    if (!(basisContract.mustFailCases ?? []).includes(failure)) {
      violations.push(violation("oma.finance.must_fail_case_missing", "docs/finance/ledger-basis-type-contract.yml", `缺少必须失败用例: ${failure}.`, { failure }));
    }
  }
  return violations;
}
...
function validateRuntimeBindings() {
  ...
  for (const token of [
    ...
    if (!ledgerSource.includes(token) && !financeSource.includes(token) && !uowSource.includes(token)) {
      violations.push(violation("oma.finance.runtime_token_missing", "services/core-api/WorkOS.Api/Runtime/LedgerSemanticRules.cs", `缺少 runtime token: ${token}.`, { token }));
    }
  }

  if (!financeSource.includes("LedgerSemanticRules.Validate(transaction, entries)")) {
    violations.push(violation("oma.finance.finance_pipeline_not_semantic", "services/core-api/WorkOS.Api/Runtime/FinanceTruthKernel.cs", "FinanceTruthPipeline.ValidateBalanced 必须调用 LedgerSemanticRules。"));
  }
  if (!uowSource.includes("LedgerSemanticRules.Validate(transaction, entries)")) {
    violations.push(violation("oma.finance.uow_not_semantic", "services/core-api/WorkOS.Api/Runtime/OperationsUnitOfWork.cs", "OperationsUnitOfWork 必须在提交前调用 LedgerSemanticRules。"));
  }
  if (!financeSource.includes("OwnerRole") || !financeSource.includes("EvidenceRefs") || !financeSource.includes("TraceRef")) {
    violations.push(violation("oma.finance.finance_case_missing_audit_fields", "services/core-api/WorkOS.Api/Runtime/FinanceTruthKernel.cs", "FinanceCase 必须携带 owner / evidence / trace。"));
  }
  return violations;
}
...
function validateNoProductionDrift() {
  ...
  if (basisContract.productionAllowed !== false || liabilityContract.productionAllowed !== false || semanticRules.productionAllowed !== false) {
    violations.push(violation("oma.finance.production_drift", "docs/finance/finance-semantic-truth-kernel.yml", "OMA finance-gate 不允许声明业务生产。"));
  }
  if (liabilityContract.failureImpact?.dormitoryL2ProductionAllowed !== false) {
    violations.push(violation("oma.finance.l2_drift", "docs/finance/deposit-liability-semantic-contract.yml", "OMA finance-gate 不允许宿舍 L2。"));
  }
  return violations;
}
