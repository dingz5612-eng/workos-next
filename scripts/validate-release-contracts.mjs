...
const sliceManifest = JSON.parse(fs.readFileSync("docs/contracts/production-slice-manifest.json", "utf8"));
...
const capabilitySurfacePolicyIds = new Set(["Dormitory.FirstGoldenChain"]);
...
for (const policy of surfacePolicy.policies || []) {
  if (!sliceIds.has(policy.sliceId) && !capabilitySurfacePolicyIds.has(policy.sliceId)) {
...
console.log("Release contract files: PASS");
