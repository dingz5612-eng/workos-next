...
function Assert-RipgrepAvailable {
  if (-not (Get-Command rg -ErrorAction SilentlyContinue)) {
    Fail "ripgrep (rg) is required for OMA source hygiene checks."
  }
}
...
Assert-NoMatches @("apps", "services", "tests") "scenarioFlows|data-task|data-scenario|taskView|objectView" "Old page/task/object model terms must not remain in current OMA source."
...
Assert-NoMatches @(".github", "docs/oma", "docs/contracts/oma.current.json") "GateResult|WON-18" "Current OMA authority surfaces must not use retired gate names."
...
Write-Host "OMA source hygiene: PASS"
exit 0
