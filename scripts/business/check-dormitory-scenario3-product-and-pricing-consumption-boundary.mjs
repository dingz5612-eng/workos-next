...
const resultPath = "artifacts/oam/checks/dormitory-scenario3-product-and-pricing-consumption-boundary-result.json";
...
if (JSON.stringify((docs.runtimeMirror.commands ?? []).map((item) => item.commandId)) !== JSON.stringify(commandIds)) {
  fail("runtime mirror must expose exactly the five scenario 3 write commands.");
}
...
if (!runtimeRulesText.includes("DormitoryScenario3ProductAndPricing.generated.json")) {
  fail("runtime rules must consume DormitoryScenario3ProductAndPricing.generated.json.");
}
if (!runtimeRulesText.includes("Scenario3ProductPricingRuntimeAdapter")) {
  fail("runtime rules must include scenario 3 generated adapter.");
}
...
for (const forbidden of ["报价", "库存锁定", "预订", "入住", "收款", "押金", "退款", "账务"]) {
  if ((docs.handoff.readSideOutputs ?? []).includes(forbidden)) fail(`scenario 3 handoff must not output ${forbidden}.`);
}
for (const forbiddenTerm of ["可报价", "可预订", "已预订", "已锁定", "已入住", "已收款", "final GO", "RatePlanConfirm", "resource-saleability", "lead-reservation", "golden-chain"]) {
...
const result = {
  version: "oam.dormitory-scenario3-product-and-pricing-consumption-boundary-check.v1",
  ...
  consumerBoundaries: {
    ...
    downstreamScenario4ReadonlySummaryOnly: true,
    failureNoSideEffects: true,
    quoteReservationInventoryWritesForbidden: true,
...
if (result.status !== "PASS") {
  console.error("Dormitory scenario 3 consumption boundary check: FAIL");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log(`Dormitory scenario 3 consumption boundary check: PASS (${scenarioDigest})`);
